...
class DeleteCartIViewModel(val appRepository: AppRepository) : ViewModel(){
    ...
    fun getReviewResponse(productid: String,userid: String) = getRatingData(productid,userid)

    fun getRatingData(productid: String,userid: String){
        _userResponse.postValue(Event(Resource.Loading()))
        try{
            disposable.add(appRepository.deleteCart(productid,userid).subscribeOn(
